...
import com.example.shopinkarts.databinding.Banner2ContainerBinding
import com.example.shopinkarts.response.Banner


class Banner2Adapter(val context: Context,val arrayList: ArrayList<Banner>) :
    RecyclerView.Adapter<Banner2Adapter.IntroSlideViewHolder>() {

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): IntroSlideViewHolder {

        val binding:Banner2ContainerBinding=DataBindingUtil.inflate(
            LayoutInflater.from(parent.context),
            R.layout.banner2_container,
            parent,
            false
        )
    ...
    override fun onBindViewHolder(holder: IntroSlideViewHolder, position: Int) {
        ...
        holder.binding.apply {
            Glide.with(context).load(itemDetails.bannerImage).into(imageSlideIV)
        }
...
//      banner type 0 for external
    ...
    class IntroSlideViewHolder(itemView: Banner2ContainerBinding) :
        RecyclerView.ViewHolder(itemView.root) {

        val binding:Banner2ContainerBinding=itemView
